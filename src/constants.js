...
export const MAX_BUILDING_HEIGHT = 12.5; // Metre cinsinden maksimum bina yüksekliği
export const FLOOR_HEIGHT = 3; // Tek bir katın standart yüksekliği (metre)
export const MAX_FLOORS_PER_BLOCK = 4; // Bir blokta izin verilen maksimum kat sayısı

// Her 3 adet 2+1 daire için 1 otopark gerekir
export const PARKING_SPACES_PER_2_PLUS_1 = 3; 

// Her 2 adet 3+1 daire için 1 otopark gerekir
export const PARKING_SPACES_PER_3_PLUS_1 = 2;

// Projeyi kaydetmek için localStorage'da kullanılacak anahtar
export const LOCAL_STORAGE_KEY = "zoningProjectData";```

---

### 2. `src/hooks/useZoningCalculator.js` (Değişiklik Yok)
Tüm mantığı barındıran bu özel hook da aynı kalmalıdır.
...
export function useZoningCalculator() {
  ...
  // ... (Bu hook'un içeriği önceki yanıttaki ile tamamen aynı)
  // handleChange, validateInputs, calculateBlocks, checkPlanRules,
  // calculate, saveProject, loadProject fonksiyonları ve useEffect
  ...
  const checkPlanRules = (apt2, apt3, buildingHeight, parkingSpaces) => {
    ...
    if (buildingHeight > MAX_BUILDING_HEIGHT) {
      warningsLocal.push(`Bina yüksekliği (${buildingHeight}m) yasal sınırı (${MAX_BUILDING_HEIGHT}m) aşıyor. Proje bloklara bölünecek.`);
    }
  ...
  const calculate = () => {
    ...
    const parkingSpaces = apt2 + apt3; // Her daireye 1 otopark
    ...
    setResults({
      ...
      apartmentCount,
      parkingSpaces,
      buildingHeight: (blocksArr[0]?.floors || 0) * FLOOR_HEIGHT,
